...
void build_patch(int32_t targetAddr, const std::vector<uint8_t>& old_bytes, int disp_offset, const uintptr_t new_ptr) {
    ...
    bool patch = true;
    //check target first before modifying
    //only write if the all opcodes match. 
    for (int i = 0; i < len; i++) 
    {
        if (*((byte*)targetAddr + i) != old_bytes[i]) {
            debug("Already patched: address: %08X actual value: %02X expected value: %02X\n",
                targetAddr + i, *((byte*)targetAddr + i), old_bytes[i]);
                patch = false;
        }
    }
    
    //check if safe to apply, logs will be used as a fallback to investigate
    // probably should of used a fully patched binary image to generate the vehpatch..... oops...
    if(patch == true)
    {
       for (int i = 0; i < len; i++) 
        {
            if (patch_bytes.size() != 0) 
...
void apply_veh_patches() {
    ...
    write_bytes(0x005c09c9,old_veh_init_bytes,new_veh_init_bytes,sizeof(new_veh_init_bytes));
    ...
    build_patch(0x0046DFF3, std::vector<uint8_t>{0x66, 0x89, 0x1C, 0x55, 0x3C, 0x28, 0x95, 0x00}, 4, new_VEH_addr + 0x14);
    // click(int,int,int) - MOV word ptr [EDX*0x2 + 0x952844],CX
    build_patch(0x0046E009, std::vector<uint8_t>{0x66, 0x89, 0x0C, 0x55, 0x44, 0x28, 0x95, 0x00}, 4, new_VEH_addr + 0x1C);
    ...
    build_patch(0x0046F18C, std::vector<uint8_t>{0x66, 0x89, 0x14, 0x4D, 0x3C, 0x28, 0x95, 0x00}, 4, new_VEH_addr + 0x14);
    // on_mouse_move(int,int) - MOV word ptr [EDX*0x2 + 0x952844],CX
    build_patch(0x0046F1A2, std::vector<uint8_t>{0x66, 0x89, 0x0C, 0x55, 0x44, 0x28, 0x95, 0x00}, 4, new_VEH_addr + 0x1C);
    ...
    build_patch(0x004D1170, std::vector<uint8_t>{0x3B, 0x34, 0x95, 0x00, 0xCC, 0x96, 0x00}, 0, new_VEH_addr + 0xC13);
